...
def get_pred_junctions(convex_map, joff_map, mask_pred):

    mask_pred = cv2.resize(mask_pred, (128,128))
    mask_pred = (mask_pred > 0.5).astype(np.uint8)

    corners = cv2.cornerHarris(mask_pred, 3, 3, 0.06)   # 3,5,0.06
    corners_ = np.zeros_like(mask_pred)
    corners_[corners > 0.01 * corners.max()] = 1
    corners_list = torch.tensor(corners_,device='cuda').unsqueeze(dim=0)
    convex_map = convex_map + corners_list

    # convex junctions
    convex_pred_nms = non_maximum_suppression(convex_map)
    topK = min(700, int((convex_pred_nms > 0.0005).float().sum().item()))   #700,0.0005
    juncs_convex, _ = get_junctions(convex_pred_nms, joff_map, topk=topK)
    juncs_convex = juncs_convex.detach().cpu().numpy()
...
def simple_polygon(poly, thres=10):
    ...
    if len(simple_poly)>0:
        simple_poly = np.concatenate((simple_poly, simple_poly[0].reshape(-1,2)))
    return simple_poly

def check_polygon(poly, mask, kernal_size):
    mask_dilate = cv2.dilate(mask, np.ones((kernal_size,kernal_size), np.uint8))   # (300,300)
    poly = [loc for loc in poly if mask_dilate[np.clip(round(loc[1]),0,299),np.clip(round(loc[0]),0,299)]==1]
    return np.array(poly)

def generate_polygon(prop, mask, juncs, pid, test_inria):
    poly, score, juncs_pred_index = get_poly(prop, mask, juncs)

    return poly, score, juncs_pred_index



def get_poly(prop, mask_pred, junctions):
    ...
    for contour, h in zip(contours, hierarchy[0]):
        ...
        if h[3] != -1:
            if cv2.contourArea(contour) >= 20:   #50
        ...
        if len(c) > 3:
            ...
            if len(junctions) > 0:
                ...
                u, ind = np.unique(cj_match_[cj_dis < 4], return_index=True)
                if len(u) > 2:
                    ppoly = junctions[u[np.argsort(ind)]]
                    ppoly = np.concatenate((ppoly, ppoly[0].reshape(-1, 2)))
                    init_poly = ppoly
            #init_poly = check_polygon(init_poly, prop_mask, 5)
            # if len(init_poly)<1:
            #     continue
            #init_poly = simple_polygon(init_poly, thres=20)   #20
