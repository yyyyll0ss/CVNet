...
def save_viz(image, polys, save_path, filename):
    ...
    for i, polygon in enumerate(polys):
        color = colormap[i % num_color]
        plt.gca().add_patch(Patches.Polygon(polygon, fill=True, facecolor=color, linewidth=1.5, alpha=0.5))
        plt.plot(polygon[:,0], polygon[:,1], color=color, marker='.')
    
    impath = save_path
    if not osp.exists(impath):
        os.makedirs(impath)
    plt.savefig(osp.join(impath, filename.replace('.png','.pdf')), bbox_inches='tight', pad_inches=0.0, dpi=500)
    plt.clf()
