...
class CVNet(nn.Module):
    ...
    def forward(self, image_CD, annotations_CD, image_BE, annotations_BE):
        if self.training:
            return self.forward_train(image_CD, annotations_CD, image_BE, annotations_BE)
        else:
            return self.forward_test(image_CD, annotations_CD, image_BE, annotations_BE)

    def forward_test(self, images_CD, annotations_CD, image_BE, annotations_BE):
    ...
    def forward_train(self, image_CD, annotations_CD, image_BE, annotations_BE):
        self.train_step += 1
        ...
        targets_BE, metas = self.encoder(annotations_BE)
        ...
        if targets_CD is not None:
            loss_dict['loss_jloc'] += self.junc_loss_bce(jloc_pred_BE, targets_BE['jloc'].squeeze(dim=1))
            loss_dict['loss_joff'] += sigmoid_l1_loss(outputs[:, :], targets_BE['joff'], -0.5, targets_BE['jloc'])
            loss_dict['loss_mask_BE'] += self.mask_loss_bce(mask_pred_BE, targets_BE['mask_BE'].squeeze(dim=1).long())
