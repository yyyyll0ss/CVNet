...
# All rights reserved.
# Modified by Bowen Cheng to augment LVIS annotations with boundary.
...
class LVIS:
    def __init__(self, annotation_path, get_boundary=False, dilation_ratio=0.02):
        ...
        Args:
            annotation_path (str): location of annotations file
            get_boundary (bool): whether to get mask boundary
    ...
    def _create_index(self):
        ...
        if self.get_boundary:
            ...
            self.logger.info('Adding `boundary` to annotations.')
            tic = time.time()
    ...
    def load_anns(self, ids=None):
        ...
        Args:
            ids (int array): integer array of annotations ids

        Returns:
            anns (dict array) : loaded annotations objects
        """
    ...
    def ann_to_mask(self, ann):
        """Convert annotations which can be polygons, uncompressed RLE, or RLE
        to binary mask.
        Args:
            ann (dict) : annotations object
