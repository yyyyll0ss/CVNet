...
class LVISEval:
    def __init__(self, lvis_gt, lvis_dt, iou_type="segm", max_dets=300, dilation_ratio=0.02):
        """Constructor for LVISEval.
        Args:
            lvis_gt (LVIS class instance, or str containing path of annotations file)
            lvis_dt (LVISResult class instance, or str containing path of result file,
            or list of dict)
            iou_type (str): segm or bbox or boundary evaluation
            max_dets (int):  max number of detections per image. The official value of max_dets for LVIS is 300.
        ...
        if isinstance(lvis_dt, LVISResults):
            ...
            self.lvis_dt = LVISResults(self.lvis_gt, lvis_dt, max_dets=max_dets)
        else:
    ...
    def _prepare(self):
        ...
        # set ignore flag
        for gt in gts:
            # COCO has this field which LVIS does not have.
            if "iscrowd" not in gt:
                gt["iscrowd"] = 0
            if "ignore" not in gt:
                gt["ignore"] = 0
            gt['ignore'] = gt['iscrowd'] or gt["ignore"]
        ...
        is_lvis = "neg_category_ids" in img_data[0]
        ...
        for dt in dts:
            ...
            # We need to take special care, COCO counts all predictions.
            if is_lvis: 
    ...
    def _prepare_freq_group(self):
        ...
        if "frequency" not in cat_data[0]:
            return freq_groups
    ...
    def evaluate(self):
        ...
        self.logger.info("Evaluate annotations type *{}*".format(self.params.iou_type))
    ...
    def compute_iou(self, img_id, cat_id):
        ...
        iscrowd = [int(g['iscrowd']) for g in gt]
        ...
        for iou_thr_idx, iou_thr in enumerate(self.params.iou_thrs):
            ...
            for dt_idx, _dt in enumerate(dt):
                ...
                for gt_idx, _ in enumerate(gt):
                    ...
                    if gt_m[iou_thr_idx, gt_idx] > 0 and not iscrowd[gt_idx]:
                        continue
    ...
    def summarize(self, show_all_iouthr=False, show_freq_groups=True, show_recall=True):
        ...
        if show_all_iouthr:
            self.results["AP55"] = self._summarize('ap', iou_thr=0.55)
            self.results["AP60"] = self._summarize('ap', iou_thr=0.60)
            self.results["AP65"] = self._summarize('ap', iou_thr=0.65)
            self.results["AP70"] = self._summarize('ap', iou_thr=0.70)
        self.results["AP75"] = self._summarize('ap', iou_thr=0.75)
        if show_all_iouthr:
            self.results["AP80"] = self._summarize('ap', iou_thr=0.80)
            self.results["AP85"] = self._summarize('ap', iou_thr=0.85)
            self.results["AP90"] = self._summarize('ap', iou_thr=self.params.iou_thrs[8])  # due to floating point
            self.results["AP95"] = self._summarize('ap', iou_thr=0.95)
